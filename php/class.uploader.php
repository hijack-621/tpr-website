	...
	private function prepareFiles(){
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                //$metas['old_name'] = substr( $field['name'][$i], 0, -(strlen(strrchr($field['name'][$i], $metas['extension']))+1) );//这样有的文件名会多出字符
                $metas['old_name'] = explode('.',$field['name'][$i])[0];//可以保证上传文件名只有一个后缀名的’.‘号时，上传文件名ok！！！
