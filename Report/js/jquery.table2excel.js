...
(function ( $, window, document, undefined ) {
        ...
        init: function () {
            ...
            e.template = {
                ...
                mid: "</x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]-->" +
                    "<style>" +
                    "table{font-family: Calibri;font-size:16px}"+
            ...
            $(e.element).each( function(i,o) {
                ...
                if(e.settings.exclude_textareas) {
                    tempRows = exclude_textareas(tempRows);
                }
    ...
    function exclude_textareas(string) {
        var _patt = /(\s+value\s*=\s*"([^"]*)"|\s+value\s*=\s*'([^']*)')/i;
        return string.replace(/<textarea[^>]*>|<\/textarea>/gi, function myFunction(x){
            var res = _patt.exec(x);
            if (res !== null && res.length >=2) {
                return res[2];
            } else {
