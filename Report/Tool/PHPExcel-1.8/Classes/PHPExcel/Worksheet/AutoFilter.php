    ...
    private static function filterTestInCustomDataSet($cellValue, $ruleSet)
    {
        ...
        foreach ($dataSet as $rule) {
            if (is_numeric($rule['value'])) {
                ...
                //    String values are always tested for equality, factoring in for wildcards (hence a regexp test1)
                $retVal    = preg_match('/^'.$rule['value'].'$/i', $cellValue);
            }
            //    If there are multiple conditions, then we need to test1 both using the appropriate join operator
            switch ($join) {
...
//                echo (($result) ? 'VALID' : 'INVALID'),PHP_EOL;
                //    If filter test1 has resulted in FALSE, exit the loop straightaway rather than running any more tests
                if (!$result) {
                    break;
                }
            }
