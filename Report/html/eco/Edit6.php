...
function load(){
	...
	var reasonedit = document.getElementById('reason').value;
	console.log(reasonedit);
	...
	$.ajax({
		...
		dataType:'json',
		url:"ECO_server.php",
		data:{flag:'Edit_load',dataid:dataid,reasonedit:reasonedit},
		success:function(msg){
			console.log(msg);
			...
			document.getElementById('reason').value = msg[0]['reason'];
			//console.log(id_arr);
		},
		error:function(msg){
		console.log(msg);
...
function Owner(){
	...
	var statu=document.getElementsByName('status1');
	//console.log(statu);
	var pass = document.getElementsByName('status');
    //var reasonedit = document.getElementById('reason').value;
	var reason = $('#reason').val();//reject时，获取输入的理由
	//document.cookie=reason+";"+"expires=Fri, 31 Dec 9999 23:59:59 GMT;path=./cookie";
	//console.log(reason);
	//console.log(pass);
//查看是选择继续还是结束本步骤
	...
	for(var j=0;j<pass.length;j++){
	    if(pass[j].checked==true){
	         var passed = pass[j].value ;
		}
	}
	//console.log(passed);接收到的pass还是reject
	...
	$.ajax({
		...
		dataType:'json',
		url:"ECO_server.php",
		data:{flag:'Edit_Owner',dataid:dataid,remark:remark,status:status,passed:passed,reason:reason},

        success:function(msg){
			console.log(msg);
           //根据返回标志位判断邮件的发送方式以及提示信息
			if(msg[0]==1116){
				Send_Mail("s6",msg[1],msg[2]);
                console.log(msg[1]);
                console.log(msg[2]);
			}else if(msg[0]==1414){
               Send_Mail("rj",msg[1],msg[2]);
                console.log(msg[1]);//  TPR
                console.log(msg[2]);//model
			}else if(msg[0]=='1040'){
				alert("finish")
			}else if(msg[0]=='1041'){
...
$.each($("textarea"), function(i, n){
    $(n).css("height", n.scrollHeight + "px");//自动跳转testarea大小
})
</script>
</head>
...
<div class="container" align="center" style="text-align:center;">
	...
	<div class="row">
		...
		<div  style="width:100%;height:auto;margin-top:-60px">
			...
			<div class="row" style="text-align:center;width:100%;margin-left:50%;transform:translateX(-50%)">

				<div  style="margin:auto;text-align:center;display:inline;margin-top:18px;" name="table-edit" id="tb-edit">
					<table class="table table-bordered" id="table-edit" style=" background-color:#FFF;width:75%;margin:auto;" >
							...
							<td colspan="4">
								...
								<div class="row">
									...
									<div class="col col-md-6">
										<div class="row">
											<label><input type="radio" name="status1" id="status_1" value="1" checked="checked"/>finish</label>
											<label><input type="radio" name="status1" id="status_0" value="0" />continue</label>
						...
						<tr>
							<td colspan="4">
								...
								<div class="row">
									<div class="col col-md-6"><span>check pass</span></div>
									<div class="col col-md-6">
										<div class="row">
											<label><input type="radio" name="status" id="status_2" value="pass	" checked="checked"/>pass&nbsp;&nbsp;&nbsp;</label>
											<label><input type="radio" name="status" id="status_3" value="reject" />reject</label>
						...
						<tr>
							<td>
								(input when choose reject)reason:
							</td>
							<td colspan="3">
								<textarea rows="3" cols="40" id="reason" style="resize: none;" onchange=""></textarea>
							</td>
						</tr>
						<tr>
							<td colspan="4">
								<button type="button" class="btn btn-primary" onclick="Owner()" id="Owner_btn">Sure</button>
							</td>
						</tr>
				...
				<div  style="text-align:center;display:none;margin-top:18px;width:660px" name="table-score" id="tb-score">
					<table class="table table-bordered" id="table-score" style=" background-color:#FFF;width:100%;margin-left:50%" >
						...
						<tr>
							<td>Remark</td>
							<td colspan="3">
						<textarea rows="3" cols="40" id="sc_mark" style="resize: none;"></textarea>
