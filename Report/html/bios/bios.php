...
function showdel() {
    ...
    if(tr[0].childNodes[10].childNodes.length==0){
        for(let i=0;i<tr.length;i++){
			...
			if(tr[i].children.length==11){
				 tr[i].childNodes[10].appendChild(del_btn);
			}else{
		...
		if(tr[i].children.length==11){
				 tr[i].childNodes[10].removeChild(tr[i].childNodes[10].childNodes[0]);
			}else{
...
<nav class="nav" style="margin-left:30px;margin-top:120px">
	<ul class="nav nav-pills" >

		<li class="nav-item">
			<a class="nav-link " href="Create.php" style="background-color:#CC6633;margin-left: 3px;color: #fff" target="_bank">Create</a>
		</li>
		<li class="nav-item">
			<a class="nav-link " href="javascript:void(0);" style="background-color:#CC6633;margin-left: 3px;color: #fff" onclick="show_page()">show page</a>
		</li>
		<li class="nav-item">
			<a class="nav-link " href="javascript:void(0);" style="background-color:#CC6633;margin-left: 3px;color: #fff" onclick="show_all()">show all</a>
		</li>


	</ul>
</nav>
<h4 style="color:black;">BIOS Control System</h4>
...
<div class="row">
	...
	<div class="col col-md-3">
        <div class="row">
            <div class="col col-md-3"><button type="button" class="btn btn-primary" style="margin-left:12px" onclick="sh_data()">search</button></div>
            <div class="col col-md-3"><button type="button" class="btn btn-primary" style="margin-left:14px" onclick="lock()">Lock</button></div>
            <div class="col col-md-3"><button type="button" class="btn btn-primary" style="margin-left:6px" onclick="unlock()">Unlock</button></div>
            <div class="col col-md-3"><button type="button" class="btn btn-primary" style="margin-left:14px" onclick="showdel()">DEL</button></div>
        </div>
	</div>
</div>
</div>
		...
		for(i=0;i<msg[x].length;i++){
			...
			if(i==0){
				...
				if(msg[x][i]['TPR']=="CGS"){
					...
					if(msg[x][i]['Status']==0||msg[x][i]['Status']==3){
					...
					}else if(msg[x][i]['2_Status']==5&&msg[x][i]['3_Status']==1){
						...
						if(msg[x][i]['Status']==0){
                         ...
                         status_td.setAttribute("bgcolor","#00FF00");
							...
							status_td.innerHTML='Close';
							status_td.setAttribute("bgcolor","#FF0000");
						}else if(msg[x][i]['Status']==3){
							if(msg[x][i]['5_Status']==1){
								status_td.innerHTML='Close';
							status_td.setAttribute("bgcolor","#FFFF00");
							}else if(msg[x][i]['5_Status']==5){
								status_td.innerHTML='Close';
							status_td.setAttribute("bgcolor","#FF0000");
							}else{
								status_td.innerHTML='Begin';
