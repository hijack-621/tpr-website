            ...
            case 'unclose':
                ...
                //die($sql);
                 ...
                 //$stpr=array("RLC_SH","CEP","CEB","IGS","CTS","TSI","CGS");
                    ...
                    $dataarr[] =array($tpr);
                    $sql = "select Model, PPID,Station,Fail_Item,Test_time,Correct,TPR_owner,Compal_owner,TPR_FA,Action,update_time from $table where isnull(Compal_check) order by Test_time";
                    //die($sql);
                    $result = mysqli_query($connID,$sql);
                    while(($row = mysqli_fetch_array($result,MYSQLI_NUM)) !=false && $row >0){
                        $arr[] = $row;
