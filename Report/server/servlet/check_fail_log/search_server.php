...
switch ($flag){
    ...
    case 2:
        ...
        if($open==null){
           ...
           // echo 'B';
            if($bgtime==null&&$edtime!=null){
                $result=mysqli_query($connID,"select * from $table1 where Test_time<='$edtime' and isnull(Compal_check)  order by Test_time");
            }elseif($bgtime!=null&&$edtime==null){
                $result=mysqli_query($connID,"select * from $table1 where Test_time>='$bgtime' and isnull(Compal_check) order by Test_time");
            }elseif ($edtime==null&&$bgtime==null){
				
                $result=mysqli_query($connID,"select * from $table1 where  Compal_check is null order by Test_time ");
				//die("select * from $table1 where  Compal_check is null order by Test_time ");
            }elseif ($edtime!=null&&$bgtime!=null){
                $result=mysqli_query($connID,"select * from $table1 where Test_time>='$bgtime' and Test_time<='$edtime' and isnull(Compal_check) order by Test_time");
            }
        }
    ...
    case 3:
        ...
        $tpr=$_SESSION['utpr'];
        $faca=new n_faca();
        $faca_dt=$faca->getdata($tpr,$type,$utpr);
