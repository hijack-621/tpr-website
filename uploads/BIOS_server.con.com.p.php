...
//         mysqli_select_db($mysqli, $dbname);
        ...
        if($S==0){
            ...
            for($i=0;$i<count($tprs);$i++){
                if(!strpos($T,$tprs[$i])){
                    ...
                    $sql="insert into bios_system(Id,Model,Activity,TPR,TPRS,Eco_no,Old_ver,Old_sysver,New_ver,New_sysver,1_Begingtime,1_Endtime,1_Remark,1_Status,1_Owner,1_Checker,1_ck_date,1_ck_day,1_Score,1_ck_mark,1_file,2_Begingtime,2_Endtime,2_Remark,2_Status,2_Owner,2_Checker,2_ck_date,2_ck_day,2_Score,2_ck_mark,2_file,3_Begingtime,3_Owner,3_Checker,3_Status,3_file) select '$tpr_Id',Model,Activity,'$tprs[$i]','$tprs[$i]',Eco_no,Old_ver,Old_sysver,New_ver,New_sysver,1_Begingtime,1_Endtime,1_Remark,1_Status,1_Owner,1_Checker,1_ck_date,1_ck_day,1_Score,1_ck_mark,1_file,2_Begingtime,2_Endtime,2_Remark,2_Status,2_Owner,2_Checker,2_ck_date,2_ck_day,2_Score,2_ck_mark,2_file,'$now_date','$Owner','$Checker',0,3_file from bios_system where Id='$Id' ";
                    $result=$mysqli->query($sql);
                    if($result!=0){
                        $T.=",".$tprs[$i];
                    }
    ...
    case 'Edit_load':
        ...
        $sql="select $Owner,$Checker,$Acty,Model,$Bgtime,$Remark,$score,$ck_mark,$day from bios_system where Id='$val[0]' ";
        $result=$mysqli->query($sql);
        $nums=mysqli_num_rows($result);
        if ($nums!=0){
            $arr=array();
                ...
                case 3:
                    ...
                    if(!$mysqli->errno){
                        for($i=0;$i<count($tprs);$i++){
                            ...
                            $Id = strtoupper ( md5 ( uniqid ( rand (), true ) ) );
                            $sql="insert into bios_system(Id,Model,Activity,TPR,TPRS,Eco_no,Old_ver,Old_sysver,New_ver,New_sysver,1_Begingtime,1_Endtime,1_Remark,1_Status,1_Owner,1_Checker,1_ck_date,1_ck_day,1_Score,1_ck_mark,1_file,2_Begingtime,2_Endtime,2_Remark,2_Status,2_Owner,2_Checker,2_ck_date,2_ck_day,2_Score,2_ck_mark,2_file,3_Begingtime,3_Owner,3_Checker,3_Status,3_file,D_bgtime) select '$Id',Model,Activity,'$tprs[$i]','$tprs[$i]',Eco_no,Old_ver,Old_sysver,New_ver,New_sysver,1_Begingtime,1_Endtime,1_Remark,1_Status,1_Owner,1_Checker,1_ck_date,1_ck_day,1_Score,1_ck_mark,1_file,2_Begingtime,2_Endtime,2_Remark,2_Status,2_Owner,2_Checker,2_ck_date,2_ck_day,2_Score,2_ck_mark,2_file,'$now_date','$Owner','$Checker',0,3_file,'$now_date' from bios_system where Id='$val[0]' ";
                            $result=$mysqli->query($sql);
                            if(!$mysqli->errno){
                                $tag[]=$tprs[$i];
                                $time[]=1;
...
//        }
	...
	*Please login TPR ManageMent System(https://www.compal.top/Report/web/BIOS_System/Show.html) for details.<br/>
